    ...
    def get(self, path: str, params: dict | None = None) -> list[dict[str, Any]]:
        ...
        while True:
            ...
            if r.status_code == 400:
                raise RuntimeError(
                    "J-Quantsから400 Bad Requestが返されました。"
                    "フリープランの取得可能期間やAPIパラメータを確認してください。"
                )
...
def normalize_codes(codes_text: str) -> list[str]:
    codes = []
...
def fetch_screening_cached(
    _api_key: str,
    codes_tuple: tuple[str, ...],
    years: int,
    override_csv: bytes | None,
):
    if override_csv:
        class UploadedLike:
            ...
            def getvalue(self):
                return self.raw
        override_obj = UploadedLike(override_csv)
    else:
        override_obj = None

    emp_map, rev_map, debt_map = parse_override(override_obj)
    jq = JQuants(_api_key)
    rows = []
    ...
    for code in codes_tuple:
        ...
        try:
            ...
            if dates:
                ...
                prices = pd.DataFrame(
                    jq.get("/equities/bars/daily", {"code": code, "from": start, "to": end})
                )
            else:
                prices = pd.DataFrame()
...
# J-Quants API key: prefer Streamlit Secrets, then manual input fallback.
secret_api_key = ""
try:
    secret_api_key = str(st.secrets.get("JQUANTS_API_KEY", "")).strip()
except Exception:
    secret_api_key = ""
...
with st.sidebar:
    ...
    if mode == "J-Quants実データ":
        if secret_api_key:
            ...
            st.success("APIキーは保存済みです。毎回の入力は不要です。")
        else:
            api_key = st.text_input("J-Quants APIキー", type="password")
            st.caption("Streamlit Secretsに保存すると次回から入力不要になります。")

    codes_text = st.text_area("証券コード", "7203, 6758, 9984")
    years = st.slider("決算統計の期間（年）", 1, 10, 1)
    override = st.file_uploader("従業員数などの補完CSV（任意）", type=["csv"])

    st.divider()
    st.caption("取得結果は7日間キャッシュします。")
    refresh = st.button("最新データに更新", use_container_width=True)
    run = st.button("スクリーニング実行", type="primary", use_container_width=True)

if refresh:
    fetch_screening_cached.clear()
    st.toast("保存済みキャッシュを削除しました。次回実行時に最新データを取得します。")

if not run:
    st.info("左側で条件を設定して「スクリーニング実行」を押してください。")
    st.stop()
...
if mode == "デモ":
    ...
    codes = normalize_codes(codes_text)
    if not codes:
    ...
    override_bytes = override.getvalue() if override else None

    with st.spinner("データを取得しています。保存済みデータがあれば再利用します…"):
        result, fetched_at = fetch_screening_cached(
            api_key,
            tuple(codes),
            int(years),
            override_bytes,
...
if mode == "J-Quants実データ":
    st.caption(f"最終取得: {fetched_at}（同じ条件の結果は最大7日間再利用）")
...
if "1人当たり売上(万円)" in filtered and min_sales_emp > 0:
    filtered = filtered[
        pd.to_numeric(filtered["1人当たり売上(万円)"], errors="coerce") >= min_sales_emp
    ]

if "有利子負債(億円)" in filtered and max_debt > 0:
    filtered = filtered[
        pd.to_numeric(filtered["有利子負債(億円)"], errors="coerce") <= max_debt
    ]

if "翌日上昇率(%)" in filtered and min_win > 0:
    filtered = filtered[
        pd.to_numeric(filtered["翌日上昇率(%)"], errors="coerce") >= min_win
    ]
...
st.caption(
    "従業員数・有利子負債はJ-Quantsの契約プランや開示データの都合で取得できない場合があります。"
    "その場合はCSV補完を利用してください。"
)
st.caption(
    "※ 7日キャッシュはStreamlit Community Cloudの再起動・休止復帰などで消える場合があります。"
    "ただし通常の画面更新や再実行では再利用されます。"
)
